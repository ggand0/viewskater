...
fn submenu_button(label: &'_ str, text_size: u16) -> button::Button<'_, Message, WinitTheme, Renderer> {
    base_button(
        row![
            text(label)
                .size(text_size)
...
pub fn build_menu(app: &'_ DataViewer) -> MenuBar<'_, Message, WinitTheme, Renderer> {
    menu_bar!(
        (
            container(
