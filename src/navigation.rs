...
use crate::pane;
use crate::image_cache::{ImageCache, LoadOperation, LoadOperationType, load_images_by_operation, load_all_images_in_queue};
use crate::pane::{Pane, get_pane_with_largest_dir_size, get_master_slider_value};
...
fn are_all_next_images_loaded(panes: &mut Vec<&mut Pane>, is_slider_dual: bool, loading_status: &mut LoadingStatus) -> bool {
    if is_slider_dual {
        panes
        .iter()
...
fn are_all_prev_images_loaded(panes: &mut Vec<&mut Pane>, is_slider_dual: bool, loading_status: &mut LoadingStatus) -> bool {
    if is_slider_dual {
        panes
        .iter()
        .filter(|pane| pane.is_selected)  // Filter only selected panes
...
pub fn are_panes_cached_next(panes: &mut Vec<&mut Pane>, _pane_layout: &PaneLayout, is_slider_dual: bool) -> bool {
    panes
    .iter()
    .filter(|pane| pane.is_selected)  // Filter only selected panes
...
pub fn are_panes_cached_prev(panes: &mut Vec<&mut Pane>, _pane_layout: &PaneLayout, is_slider_dual: bool) -> bool {
    panes
    .iter()
    .filter(|pane| pane.is_selected)  // Filter only selected panes
...
pub fn load_next_images_all(panes: &mut Vec<&mut Pane>, pane_indices: Vec<usize>, loading_status: &mut LoadingStatus, _pane_layout: &PaneLayout, is_slider_dual: bool) -> Command<Message> {
    let target_indices = get_target_indices(panes);

    if target_indices.is_empty() {
        return Command::none();
    }

    if let Some((next_image_indices_to_load, is_image_index_within_bounds)) = calculate_loading_conditions(panes) {
        ...
        if should_enqueue_loading(is_image_index_within_bounds, &loading_status, &next_image_indices_to_load, &load_next_operation, panes) {
            enqueue_loading_operations(panes, pane_indices, target_indices, loading_status, load_next_operation);
...
fn calculate_loading_conditions(panes: &Vec<&mut Pane>) -> Option<(Vec<isize>, bool)> {
    ...
    if next_image_indices_to_load.is_empty() {
        ...
        Some((next_image_indices_to_load, is_image_index_within_bounds))
    }
}

fn should_enqueue_loading(is_image_index_within_bounds: bool, loading_status: &LoadingStatus, next_image_indices_to_load: &Vec<isize>, load_next_operation: &LoadOperation, panes: &mut Vec<&mut Pane>) -> bool {
...
fn enqueue_loading_operations(
    panes: &mut Vec<&mut Pane>,
    pane_indices: Vec<usize>,
    target_indices: Vec<isize>,
    loading_status: &mut LoadingStatus,
    load_next_operation: LoadOperation
) {
    loading_status.enqueue_image_load(load_next_operation);
}

fn get_target_indices(panes: &mut Vec<&mut Pane>) -> Vec<isize> {
...
pub fn load_prev_images_all(panes: &mut Vec<&mut Pane>, pane_indices: Vec<usize>, loading_status: &mut LoadingStatus, _pane_layout: &PaneLayout, is_slider_dual: bool) -> Command<Message> {
    ...
    if let Some((prev_image_indices_to_load, is_image_index_within_bounds)) = calculate_loading_conditions_prev(panes) {
        ...
        if should_enqueue_loading(is_image_index_within_bounds, &loading_status, &prev_image_indices_to_load, &load_prev_operation, panes) {
            enqueue_loading_operations_for_previous(panes, pane_indices, target_indices,loading_status, load_prev_operation);
            return load_images_by_operation(panes, loading_status);
        }
    }
...
fn calculate_loading_conditions_prev(panes: &Vec<&mut Pane>) -> Option<(Vec<isize>, bool)> {
    ...
    if prev_image_indices_to_load.is_empty() {
        ...
        Some((prev_image_indices_to_load, is_image_index_within_bounds))
    }
}
...
fn enqueue_loading_operations_for_previous(
    panes: &mut Vec<&mut Pane>,
    pane_indices: Vec<usize>,
    target_indices: Vec<isize>,
    loading_status: &mut LoadingStatus,
...
pub fn move_right_all(panes: &mut Vec<pane::Pane>, loading_status: &mut LoadingStatus, slider_value: &mut u16,
    ...
    if !are_panes_cached_next(&mut panes_to_load, pane_layout, is_slider_dual) {
        ...
        for (cache_index, pane) in panes_to_load.iter_mut().enumerate() {
